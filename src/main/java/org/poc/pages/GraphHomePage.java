...
public class GraphHomePage extends BasePage {
    ...
    public void validateEmpGrowthPerYear() {
        List<WebElement> elements = webOp.getElements(getLocator("employeeGrowthYearNodes"));
        // List<WebElement> elements =
        // driver.findElements(getBy("employeeGrowthYearNodes"));
        Actions actions = new Actions(driver);
        for (WebElement element : elements) {
            actions.moveToElement(element).perform();
            WebElement tooltip = webOp.getElement(getLocator("employeeGrowthYearNodesTooltip"));
            try {
                Thread.sleep(100);
            } catch (Exception e) {
            }
            webOp.log().info("Tool tip info {}", tooltip.getText());
        }
    }
    ...
    public void logRevenueAndCompare(Map<String, Object> data) {
        ...
        List<WebElement> bars = webOp.getElements(getLocator("monthBar"));
        List<WebElement> tooltips = webOp.getElements(getLocator("monthTooltip"));
        Actions actions = new Actions(driver);
        ...
        if (Integer.parseInt(r1) > Integer.parseInt(r2)) {
            webOp.log().info(between[0] + " has higher revenue: " + r1 + " than " + between[1] + " revenue: " + r2);
        } else if (Integer.parseInt(r2) > Integer.parseInt(r1)) {
            webOp.log().info(between[1] + " has higher revenue: " + r2 + " than " + between[0] + " revenue: " + r1);
        } else {
            webOp.log().info(between[0] + " and " + between[1] + " have equal revenue: " + r1);
        }
    }
}
